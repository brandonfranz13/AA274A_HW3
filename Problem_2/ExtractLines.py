...
def SplitLinesRecursive(theta, rho, startIdx, endIdx, params):
    ...
    alpha = np.concatenate([np.array(alpha1, ndmin=1), np.array(alpha2, ndmin=1)])
    r = np.concatenate([np.array(r1, ndmin=1), np.array(r2, ndmin=1)])
    idx = np.vstack((i1, i2))
    ########## Code ends here ##########
    return alpha, r, idx
...
def FindSplit(theta, rho, alpha, r, params): ##FAILED##
    ...
    maxIdx = np.zeros(0)
    for splitIdx in range(params['MIN_POINTS_PER_SEGMENT'], len(theta)-params['MIN_POINTS_PER_SEGMENT']+1):
        distance = np.abs((rho[splitIdx]*np.cos(theta[splitIdx] - alpha) - r))
        if distance >= params['LINE_POINT_DIST_THRESHOLD']:
            possibleSplits = np.vstack((possibleSplits, [splitIdx, distance]))
...
def MergeColinearNeigbors(theta, rho, alpha, r, pointIdx, params):
    ...
    for i in range(1, N_lines):
        ...
        if s < 0: #accept merge
            ...
            alphaOut = np.append(alphaOut, (alpha[i-1], alpha[i]))
            rOut = np.append(rOut, (r[i-1], r[i]))
            pointIdxOut = np.vstack((pointIdxOut, pointIdx[i-1], pointIdx[i]))
